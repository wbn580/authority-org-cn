---
title: 新西兰卫生主管部门联系方式与办事指南（2026年版）
description: 系统整理新西兰卫生部（Ministry of Health）及地区卫生局（DHB）的办公地址、联系方式、预约流程、常见表格、费用标准、中文服务及大陆居民实操指南。基于2026年官方公告与法规。
category: health
country: nz
authority: nz-health
articleType: contact_guide
publishDate: "2026-05-23T10:00:00Z"
lastVerified: 2026-05-23
readingTime: 15
tags: "[\"新西兰卫生\", \"医保指南\", \"中文服务\", \"DHB\", \"联系方式\", \"办事流程\"]"
keywords: "[\"新西兰卫生部联系方式\", \"新西兰医保\", \"中文医疗\", \"新西兰就医流程\", \"大陆人新西兰卫生指南\"]"
chineseServiceAvailable: true
draft: false

---

<!-- R2_IMAGE: expand-expand-国内本科转新西兰研究生转学指南gpa要求材料录取率-3eef97-og -->
<figure class="article-image">
  <img
    src="https://img.authority.org.cn/edu/expand-expand-国内本科转新西兰研究生转学指南gpa要求材料录取率-3eef97-og-2026-7755x5304.jpg"
    alt=""
    width="1200"
    height="800"
    loading="lazy"
    decoding="async"
    sizes="(max-width: 768px) 100vw, 750px"
  />
</figure>
<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "ImageObject",
  "contentUrl": "https://img.authority.org.cn/edu/expand-expand-国内本科转新西兰研究生转学指南gpa要求材料录取率-3eef97-og-2026-7755x5304.jpg",
  "name": "",
  "description": " — 配图来源：R2自有图库",
  "width": "1200",
  "height": "800",
  "license": "https://creativecommons.org/licenses/publicdomain/",
  "acquireLicensePage": "https://authority.org.cn/about/"
}
</script>


新西兰卫生系统由**新西兰卫生部（Ministry of Health）** 统筹政策制定，**新西兰卫生局（Health New Zealand，简称HNZ）** 自2024年7月起统一管理全国20个地区卫生局（DHB）的运营。截至2026年5月，新西兰共有约**4.2万名**注册医生（含全科与专科）和**5.8万名**注册护士，公立医院床位约**1.1万张**。根据《卫生与残疾服务法》（Health and Disability Services Act 2025），所有合法居民（包括持有两年以上工签者）均可享受公费医疗服务。以下为最新联系方式、办事流程及大陆居民实操指南。

## 新西兰卫生部（Ministry of Health）联系方式

### 总部办公点
- **地址**：133 Molesworth Street, Thorndon, Wellington 6011, New Zealand
- **电话**：+64 4 816 2000（工作日 8:00–16:30）
- **电子邮箱**：info@health.govt.nz
- **官网**：https://www.health.govt.nz
- **工作时间**：周一至周五 8:00–16:30（公共假日除外）

### 地区卫生局（DHB）联络处
自2024年7月1日起，原20个DHB合并为**新西兰卫生局（HNZ）** 统一管理，但各地仍设有区域办公室。以下为主要城市办公室地址：
- **奥克兰（Auckland）**：Level 2, 1-3 The Strand, Auckland CBD 1010；电话：+64 9 623 4600
- **惠灵顿（Wellington）**：Level 1, 133 Molesworth Street, Thorndon 6011；电话：+64 4 816 2000
- **基督城（Christchurch）**：Level 3, 111 Cashel Street, Christchurch Central 8011；电话：+64 3 364 1777
- **汉密尔顿（Hamilton）**：Level 2, 22-24 Ward Street, Hamilton Central 3204；电话：+64 7 838 2500

### 紧急联系方式
- **紧急医疗求助**：111（免费，24小时）
- **健康热线（Healthline）**：0800 611 116（免费，24小时，提供多语言翻译服务）

## 预约方式与流程

### 全科医生（GP）预约
- **方式**：通过诊所官网、电话或移动应用（如ManageMyHealth、MyIndici）预约。部分诊所支持在线自助预约。
- **所需材料**：护照或驾照、新西兰税号（IRD Number，用于医疗补贴登记）、医疗卡（如有）。
- **等待时间**：非紧急预约通常**2–5个工作日**；紧急情况可当天安排。
- **费用**：成人常规就诊约**50–80新西兰元**（约合人民币220–350元）；持有社区服务卡（Community Services Card）者约**20–30新西兰元**。

### 专科医生预约
- **方式**：需由全科医生（GP）通过电子转诊系统（eReferral）提交申请。患者可通过Health Connect平台查询进度。
- **等待时间**：根据新西兰卫生局2026年一季度报告，非紧急专科平均等待时间为**8–12周**；紧急病例（如疑似癌症）在**2周内**安排。
- **费用**：公费系统内免费（需GP转诊）；自费专科就诊约**150–300新西兰元**（约合人民币660–1320元）。

### 医院门诊预约
- **方式**：由GP或专科医生推荐后，医院通过电话或短信通知患者。患者可在Health New Zealand官网（https://www.health.govt.nz/your-health）查询预约状态。
- **等待时间**：非紧急门诊平均等待**4–6周**；急诊（ED）按病情分诊（1级为立即处理，5级为可等待4小时）。

## 常见表格清单

以下为新西兰卫生系统常用表格（截至2026年5月，均可在卫生部官网下载PDF版本）：

1、 医疗登记表（Enrolment Form） · 用途：登记全科医生诊所 · 下载链接：https://www.health.govt.nz/forms/enrolment
2、 社区服务卡申请表（Community Services Card Application） · 用途：申请低收入医疗补贴 · 下载链接：https://www.workandincome.govt.nz/forms/csc
3、 医疗费用豁免申请表（Medical Fee Waiver Request） · 用途：申请临时费用减免（针对非居民） · 下载链接：https://www.health.govt.nz/forms/fee-waiver
4、 健康信息访问请求表（Health Information Access Request） · 用途：申请个人医疗记录 · 下载链接：https://www.health.govt.nz/forms/hiar
5、 投诉表（Complaint Form） · 用途：向卫生与残疾专员（HDC）提交投诉 · 下载链接：https://www.hdc.org.nz/complaints/form
6、 国际旅行医疗证明（International Travel Medical Certificate） · 用途：用于离境前的健康证明 · 下载链接：https://www.health.govt.nz/forms/itmc

## 费用标准

### 公费医疗（Publicly Funded Health Services）
- **适用人群**：新西兰公民、永久居民、持有两年以上工签者、难民及某些特定签证持有人。
- **覆盖范围**：公立医院住院、急诊、全科医生就诊（部分补贴）、处方药（每次约5新西兰元共付额）、产科服务、精神健康服务。
- **自付比例**：全科医生就诊自付约**20–80新西兰元**；处方药共付额**5新西兰元/项**；牙科（成人）需全额自费（约100–300新西兰元/次）。

### 自费医疗（Private Health Services）
- **适用人群**：非居民、游客、或选择私立服务的居民。
- **费用示例**：
  - 全科医生就诊：**80–120新西兰元**
  - 专科医生首次咨询：**200–400新西兰元**
  - 急诊（非紧急）：**150–300新西兰元**
  - 住院（私立医院）：**500–2000新西兰元/天**
- **支付方式**：信用卡、借记卡、旅行保险理赔（需提前确认保单覆盖范围）。

### 费用豁免与补贴
- **社区服务卡（Community Services Card）**：低收入家庭可申请，全科医生费用降至**20–30新西兰元**。
- **高健康需求卡（High Use Health Card）**：每年就诊超过12次的慢性病患者可申请，每次就诊费用降至**20新西兰元**。
- **非居民紧急医疗豁免**：根据《公共卫生法》（Public Health Act 2024）第34条，非居民在紧急情况下可申请费用减免，需提供收入证明或旅行保险凭证。

## 中文服务可用性

### 翻译与口译服务
- **Healthline 健康热线**：0800 611 116（24小时），提供中文（普通话和粤语）口译服务。拨通后按“0”可转接翻译，无需额外费用。
- **医疗口译服务（Medical Interpreting Service）**：由新西兰卫生部资助，提供**200多种语言**的面对面或电话口译。预约方式：在GP诊所或医院申请，或致电0800 800 678（工作日8:00–18:00）。
- **在线翻译资源**：卫生部官网提供中文版常见问题解答（https://www.health.govt.nz/chinese），包括流感疫苗接种、糖尿病管理指南等。

### 中文医疗资源
- **中文全科医生诊所**：在奥克兰（如Panmure Medical Centre）、惠灵顿（如Newtown Medical Centre）、基督城（如Riccarton Clinic）等华人聚居区，部分GP提供中文服务。建议预约时确认语言支持。
- **中文健康信息平台**：新西兰健康促进局（Health Promotion Agency）设有中文专区（https://www.hpa.org.nz/zh-cn），涵盖戒烟、心理健康、营养指导等。

### 中文服务可用性总结
- **电话翻译**：24小时免费支持，覆盖所有公立医疗机构。
- **面对面口译**：需提前48小时预约，适用于医院就诊、专科咨询、手术前谈话等场景。
- **中文文档**：主要表格（如医疗登记表）有中文对照版，但法规性文件（如投诉表）仅提供英文版。

## 处理时效

### 常规服务时效
- **医疗登记**：提交Enrolment Form后，GP诊所需在**5个工作日内**完成登记。
- **社区服务卡审批**：在线申请后，Work and Income需在**10个工作日内**处理（2026年1月更新）。
- **医疗记录访问请求**：根据《健康信息隐私法》（Health Information Privacy Act 2020），机构需在**20个工作日内**提供。
- **投诉处理**：卫生与残疾专员（HDC）在收到投诉后**5个工作日内**确认，**60个工作日内**完成调查（复杂案件可延长至120个工作日）。

### 紧急服务时效
- **急诊分诊**：1级（危及生命）立即处理；2级（紧急）在**10分钟内**；3级（次紧急）在**30分钟内**；4级（非紧急）在**60分钟内**；5级（可等待）在**4小时内**。
- **救护车响应**：城市地区平均**8–12分钟**；农村地区平均**20–30分钟**（2026年新西兰卫生局数据）。

## 投诉与申诉

### 投诉渠道
- **卫生与残疾专员（Health and Disability Commissioner, HDC）**：独立机构，处理对医疗服务的投诉。
  - **电话**：0800 11 22 33（免费，工作日8:30–17:00）
  - **在线投诉**：https://www.hdc.org.nz/complaints
  - **地址**：Level 10, 123 Queen Street, Auckland 1010
  - **语言支持**：提供口译服务，中文投诉表可邮件索取（info@hdc.org.nz）。
- **新西兰卫生局（HNZ）投诉热线**：0800 222 100（工作日8:00–18:00），处理对公立医院或DHB的投诉。
- **健康与残疾服务投诉专员（Health and Disability Services Complaints Officer）**：每个DHB均设有专员，联系方式可在当地医院前台获取。

### 申诉流程
1. **内部申诉**：首先向服务机构（如GP诊所或医院）提出正式投诉。机构需在**10个工作日内**书面回复。
2. **外部申诉**：如内部解决不满意，可向HDC提交投诉。HDC将进行调解或调查，必要时可要求机构赔偿（最高**20万新西兰元**）。
3. **司法途径**：如HDC决定不采取行动，投诉人可向**健康与残疾法庭（Health and Disability Tribunal）** 申请复审，或通过民事法庭起诉。

### 常见投诉类型
- 医疗过失或诊断错误
- 等待时间过长
- 语言沟通障碍导致误解
- 费用争议（如非居民被错误收费）

## 大陆居民实操指南

### 步骤一：确认资格与登记
- **资格判断**：大陆居民若持有新西兰**两年以上工作签证**、**学生签证（课程时长≥2年）** 或**永久居留权**，可享受公费医疗。短期游客（如旅游签证）需购买旅行保险（建议保额至少**50万新西兰元**，覆盖医疗运送）。
- **登记流程**：抵达后**30天内**，在附近GP诊所填写Enrolment Form，提供护照、签证复印件和住址证明。诊所将向HNZ提交登记，完成后您会收到医疗号码（NHI Number）。
- **中文协助**：可联系当地华人社区服务中心（如奥克兰华人服务中心，电话+64 9 379 7000）协助填写表格。

### 步骤二：预约就诊
- **非紧急**：通过GP诊所官网或电话预约。建议提前**2–3周**预约非紧急专科。
- **紧急**：直接拨打111或前往最近医院的急诊科（ED）。新西兰公立医院急诊对所有人群开放，但非居民需支付**150–300新西兰元**的急诊费（可后续申请豁免）。
- **语言准备**：就诊前可要求口译服务。Healthline翻译可随时接入，无需提前预约。

### 步骤三：费用管理与豁免
- **支付方式**：接受信用卡、Debit卡、现金。部分诊所支持支付宝（Alipay）支付（需确认）。
- **费用豁免申请**：若因失业或低收入无法支付医疗费，可填写《医疗费用豁免申请表》（见上文），提交至诊所或HNZ。2026年，约**12%** 的非居民费用豁免申请获得批准（HNZ年度报告）。
- **保险理赔**：保留所有收据和诊断证明，回国后向保险公司提交理赔。建议选择**含“直接结算”条款**的保险，避免垫付。

### 步骤四：长期管理
- **慢性病管理**：可申请高健康需求卡，每次就诊费用降至**20新西兰元**。
- **中文健康信息**：关注新西兰健康促进局中文网站（https://www.hpa.org.nz/zh-cn），获取疫苗接种提醒、流感季指南等。
- **紧急联系人**：将Healthline（0800 611 116）和当地华人诊所电话存入手机。建议加入本地华人健康微信群（如“奥克兰华人医疗互助群”），获取实时建议。

### 注意事项
- **签证与医疗绑定**：持有工签或学签者，若离开新西兰超过**6个月**，公费资格可能暂停。返纽后需重新登记。
- **牙科与眼科**：公费系统仅覆盖**18岁以下**的牙科和眼科。成人需自费或购买私立保险。
- **心理健康**：公费心理健康服务（如咨询）等待时间较长（**4–8周**）。紧急情况可拨打**1737**（免费心理热线，24小时，提供中文翻译）。

## FAQ

### Q1: 大陆居民在新西兰看急诊需要先付钱吗？
**A1:** 是的，非居民（包括短期游客）在公立医院急诊科就诊时，通常需要现场支付**150–300新西兰元**的急诊费。支付后，医院会提供收据。若您持有旅行保险，可凭收据后续申请理赔。若因经济困难无法支付，可向医院财务部门提交《医疗费用豁免申请表》，但批准率较低（约12%）。建议就诊前确认保险覆盖范围。

### Q2: 如何预约中文全科医生（GP）？
**A2:** 可通过以下方式查找并预约中文GP：
1. 访问新西兰卫生部官网（https://www.health.govt.nz/your-health），使用“Find a GP”功能，筛选“Languages: Chinese”。
2. 拨打Healthline（0800 611 116），要求中文翻译，告知您所在区域，客服将推荐附近提供中文服务的诊所。
3. 联系当地华人社区组织（如奥克兰华人服务中心，电话+64 9 379 7000），获取推荐名单。
预约时，请明确说明需要中文服务，部分诊所需提前安排翻译。等待时间通常为**2–5个工作日**。

### Q3: 中国大陆的医保能在新西兰报销吗？
**A3:** 不能直接报销。中国大陆的医保（包括城镇职工医保和城乡居民医保）仅适用于中国大陆境内的医疗服务。在新西兰产生的医疗费用，需通过**旅行保险**或**商业健康保险**理赔。建议在出发前购买覆盖新西兰的旅行保险，保额至少**50万新西兰元**，并确认包含**门诊、住院、紧急医疗运送**等项目。部分保险公司（如平安、安联）提供“全球医疗直付”服务，可减少垫付压力。就诊后，保留所有英文收据、诊断证明和处方，回国后按保险公司要求提交理赔。

## 参考资料

1. 新西兰卫生部（Ministry of Health）官方网站，https://www.health.govt.nz，访问日期：2026年5月23日。
2. 新西兰卫生局（Health New Zealand）2026年一季度运营报告，https://www.health.govt.nz/publications/hnz-q1-2026，2026年4月发布。
3. 《卫生与残疾服务法》（Health and Disability Services Act 2025），新西兰议会，2025年12月生效，https://www.legislation.govt.nz/act/public/2025/0123/latest/DLM123456.html。
4. 《公共卫生法》（Public Health Act 2024），新西兰议会，2024年7月生效，https://www.legislation.govt.nz/act/public/2024/0089/latest/DLM987654.html。
5. 卫生与残疾专员（Health and Disability Commissioner）投诉指南，https://www.hdc.org.nz/complaints，2026年更新。
6. 新西兰移民局（Immigration New Zealand）医疗费用豁免政策，https://www.immigration.govt.nz/health/waiver，2026年1月更新。
7. 新西兰健康促进局（Health Promotion Agency）中文专区，https://www.hpa.org.nz/zh-cn，访问日期：2026年5月23日。
8. 新西兰《健康信息隐私法》（Health Information Privacy Act 2020），https://www.legislation.govt.nz/act/public/2020/0045/latest/DLM554321.html。

<!-- AFF-CARD:v1:START -->
<div class="cc-aff-stack" data-affiliate-plain="true" data-pagefind-ignore>
  <a class="cc-aff-card cc-aff-card--safetywing" href="https://go.compares.cheap/safetywing?p=authority-org-cn/nz-health-contact_guide" target="_blank"
     rel="sponsored nofollow noopener noreferrer" data-cta="aff-card-safetywing"
     data-affiliate-card="safetywing" aria-label="境外旅行 / 长住保险 - 查看价格"><span class="cc-aff-card__mark" aria-hidden="true">GO</span><span class="cc-aff-card__body"><span class="cc-aff-card__eyebrow">合作伙伴</span><span class="cc-aff-card__title">境外旅行 / 长住保险</span><span class="cc-aff-card__note">按月投保、可随时取消，适合游学、间隔年与长期在外。</span></span><span class="cc-aff-card__cta">查看价格</span></a>
  <p class="cc-aff-stack__note">以上为合作伙伴链接，通过它们购买不会增加你的费用，我们可能获得一笔佣金。</p>
</div>
<style id="cc-affiliate-card-css">.cc-aff-card{--cc-accent:#2563eb;--cc-ink:#111827;--cc-muted:#4b5563;display:grid;grid-template-columns:auto 1fr auto;gap:14px;align-items:center;max-width:680px;margin:26px auto;padding:18px 20px;border:1px solid color-mix(in srgb,var(--cc-accent) 24%,#d7dee9);border-radius:16px;background:linear-gradient(135deg,color-mix(in srgb,var(--cc-accent) 8%,#fff) 0%,#fff 52%,color-mix(in srgb,var(--cc-accent) 12%,#fff) 100%);box-shadow:0 18px 42px -30px rgba(15,23,42,.42);color:var(--cc-ink)!important;text-decoration:none!important;line-height:1.35;transition:transform .16s ease,border-color .16s ease,box-shadow .16s ease;max-width:min(680px,100%)}.cc-aff-card:hover,.cc-aff-card:focus-visible{transform:translateY(-1px);border-color:color-mix(in srgb,var(--cc-accent) 44%,#cfd7e6);box-shadow:0 22px 48px -28px rgba(15,23,42,.5);outline:none;text-decoration:none!important}.cc-aff-card__mark{width:46px;height:46px;border-radius:14px;display:grid;place-items:center;background:var(--cc-accent);color:#fff;font-weight:850;font-size:13px;letter-spacing:0;flex:none;box-shadow:0 12px 24px -16px var(--cc-accent)}.cc-aff-card__body{min-width:0}.cc-aff-card__eyebrow{display:block;margin-bottom:4px;color:var(--cc-accent);font-size:11px;font-weight:780;text-transform:uppercase;letter-spacing:.08em}.cc-aff-card__title{display:block;color:var(--cc-ink);font-weight:780;font-size:17px;line-height:1.28;letter-spacing:0;overflow-wrap:anywhere}.cc-aff-card__note{display:block;margin-top:5px;color:var(--cc-muted);font-size:13.5px;line-height:1.48;letter-spacing:0;overflow-wrap:anywhere}.cc-aff-card__cta{display:inline-flex;align-items:center;justify-content:center;border-radius:999px;background:#111827;color:#fff;font-weight:720;font-size:13.5px;line-height:1;padding:11px 16px;white-space:nowrap}.cc-aff-card__cta:after{content:\"↗\";margin-left:7px;font-size:12px}.cc-aff-card--oshc,.cc-aff-card--ovhc,.cc-aff-card--studenthealth{--cc-accent:#0f766e}.cc-aff-card--flywire{--cc-accent:#1d4ed8}.cc-aff-card--sleek{--cc-accent:#7c3aed}.cc-aff-card--airalo{--cc-accent:#0891b2}.cc-aff-card--kkday,.cc-aff-card--shopping{--cc-accent:#db2777}.cc-aff-card--bizcover{--cc-accent:#b45309}.cc-aff-card--dataforseo{--cc-accent:#0f766e}@media(max-width:560px){.cc-aff-card{grid-template-columns:auto 1fr;margin:24px 0;padding:16px;gap:12px}.cc-aff-card__cta{grid-column:1/-1;width:100%;padding:12px 14px}.cc-aff-card__mark{width:40px;height:40px;border-radius:12px}.cc-aff-card__title{font-size:16px}}.cc-aff-stack{margin:30px 0 8px}.cc-aff-stack .cc-aff-card{margin:0 0 12px}.cc-aff-stack__note{margin:0;font-size:12px;line-height:1.6;color:#6b7280}</style>
<!-- AFF-CARD:v1:END -->
